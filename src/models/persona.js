...
personas.getPersonaById = function(id,callback)
{
    ...
    if (mysqlConnection) 
	{    
    mysqlConnection.query("SELECT ID_PERSONA,PRIMER_NOMBRE, SEGUND_NOMBRE, PRIMER_APELLIDO, SEGUND_APELLIDO, TIPO_IDENTIFICACION, IDENTIFICACION, DATE_FORMAT(FECHA_NACIMIENTO,'%Y-%m-%d'), GENERO, ESTADO_CIVIL, CORREO, CELULAR_1, CELULAR_2, TELEFONO_FIJO, DIRECCION_CASA, ID_BARRIO, EMPRESA, TELEFONO_EMPRESA, DATE_FORMAT(FECHA_BAUTIZO,'%Y-%m-%d'), ID_OCUPACION, ID_PROFESION, ID_MINISTERIO, TIPO_PERSONA FROM PERSONAS where IDENTIFICACION = ?",[id],(err, rows, fields) =>{
        if(!err){
